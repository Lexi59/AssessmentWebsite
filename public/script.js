        ...
        for(let child of children){
            ...
            for(var i = 0; i < assessments.length; i++){
                ...
                if(currentTest==0){row.insertCell().appendChild(document.createTextNode(""));}
                    ...
                    row.insertCell().appendChild(document.createTextNode(months[new Date(currentTestTime).getMonth()]));
                }
            }
        }
    });
    ...
    getStudents().then(children =>{
      getTests().then(tests =>{
          for(let child of children){
            ...
            var currentTest = "";
            for(var j = 0; j < tests.length; j++){
                if(tests[j].assessment == assessment && child.name == tests[j].currentChild && tests[j].timestamp > currentTestTime){
                    currentTest = tests[j];
                    currentTestTime = parseInt(tests[j].timestamp);
                }
            }
...
function finishAssessment(assessment, questions){
    ...
    //send to database
    const data = {currentChild, assessment, questions, currentAnswers, notes, correct};
    const options = {
        method: 'POST',
        body: JSON.stringify(data),
        headers:{
    ...
    fetch('/tests', options).then(response => {
